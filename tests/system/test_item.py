...
class ItemTest(BaseTest):
    def setUp(self):
        super(ItemTest, self).setUp() #get a super class and then call the method on it
        with self.app() as client:
            with self.app_context():
                ...
                self.access_token = 'JWT ' + auth_token # OR f'JWT {auth_token}'
    ...
    def test_create_duplicate_item(self):
        with self.app() as client:
            with self.app_context():
                ...
                self.assertDictEqual({'message': "An item with name \'test\' already exists."}, # \'test\' replaces '{}' in item.py line 26
                                     json.loads(resp.data.decode('utf-8')))
